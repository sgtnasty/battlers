...
/// Randomly selects a single word (a string separated by whitespace) from a given string.
///
/// # Arguments
///
/// * `text` - The input string containing words separated by whitespace.
///
/// # Returns
///
/// * An `Option<&str>` which is `Some(&str)` containing the randomly selected word,
///   or `None` if the input string is empty or contains only whitespace.
fn random_word_from_text<'a>(text: &'a str, rng: &mut ThreadRng) -> Option<&'a str> {
    // Collect the words into a `Vec`. This Vec can be used as a slice.
    let words: Vec<&str> = text.split_whitespace().collect();

    // Here, we are calling `choose` on the `words` Vec. The compiler sees this and
    // automatically dereferences the Vec to a slice (`&[&str]`), which has
    // the `choose` method available from the imported `SliceRandom` trait.
    words.choose(rng).copied()
}
